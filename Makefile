...
# Пересборка и перезапуск контейнеров для prod окружения
build-prod:
...
migrate:
	@if [ -z "$(MIGRATION_NAMES)" ]; then \
		...
		docker compose -f docker-compose.yml -f docker-compose.dev.yml exec app npx sequelize db:migrate; \
	else \
		echo "Запуск указанных миграций"; \
		for migration in $(MIGRATION_NAMES); do \
			echo "Запуск миграции: $$migration"; \
			docker compose -f docker-compose.yml -f docker-compose.dev.yml exec app npx sequelize db:migrate --name $$migration; \
		done; \
	fi
...
migrate-prod:
	@if [ -z "$(MIGRATION_NAMES)" ]; then \
		echo "Запуск всех миграций"; \
		docker compose -f docker-compose.yml -f docker-compose.prod.yml exec app-prod npx sequelize db:migrate; \
	else \
		echo "Запуск указанных миграций"; \
		for migration in $(MIGRATION_NAMES); do \
			echo "Запуск миграции: $$migration"; \
			docker compose -f docker-compose.yml -f docker-compose.prod.yml exec app-prod npx sequelize db:migrate --name $$migration; \
		done; \
	fi
...
migrate-undo:
	@if [ -z "$(MIGRATION_NAME)" ]; then \
		...
		docker compose -f docker-compose.yml -f docker-compose.dev.yml exec app npx sequelize-cli db:migrate:undo; \
	else \
		echo "Отмена миграции: $(MIGRATION_NAME)"; \
		docker compose -f docker-compose.yml -f docker-compose.dev.yml exec app npx sequelize-cli db:migrate:undo --name $(MIGRATION_NAME); \
	fi
...
migrate-undo-all:
	@if [ -z "$(MIGRATION_NAME)" ]; then \
		...
		docker compose -f docker-compose.yml -f docker-compose.dev.yml exec app npx sequelize-cli db:migrate:undo:all; \
	else \
		echo "Отмена всех миграций до: $(MIGRATION_NAME)"; \
		docker compose -f docker-compose.yml -f docker-compose.dev.yml exec app npx sequelize-cli db:migrate:undo:all --name $(MIGRATION_NAME); \
	fi
...
seed:
	docker compose -f docker-compose.yml -f docker-compose.dev.yml exec app npx sequelize-cli db:seed:all
